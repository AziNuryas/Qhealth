...
@section('title', 'Daftar Jawaban - Qhealth')
...
<div class="container-fluid py-4">
    ...
    <div class="hero-section text-center text-white position-relative">
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <h1 class="page-title">
                    <i class="bi bi-chat-square-quote-fill me-3" style="color: rgba(255,255,255,0.95);"></i>
                    Kelola Jawaban Premium
                </h1>
                <p class="subtitle">Dashboard administratif untuk mengelola jawaban kesehatan profesional dengan performa tinggi dan fitur lengkap</p>
                <div class="d-flex justify-content-center gap-3">
                    <a href="{{ route('admin.index') }}" class="premium-btn">
                        <i class="bi bi-arrow-left"></i>Kembali ke Dashboard
                    </a>
                    <a href="{{ route('admin.questions.index') }}" class="premium-btn">
                        <i class="bi bi-chat-square-text"></i>Lihat Pertanyaan
                    </a>
                </div>
            </div>
        </div>
    ...
    @if($answers && $answers->count() > 0)
        <div class="stats-grid">
                ...
                <div class="stat-number">
                    @php
                        ...
                        foreach($answers as $answer) {
                            if (
                                (isset($answer->status) && $answer->status === 'approved') ||
                                (isset($answer->is_approved) && $answer->is_approved) ||
                                (isset($answer->approved) && $answer->approved) ||
                                (!isset($answer->status) && !isset($answer->is_approved)) // Default approved
                            ) {
                                $approvedCount++;
                            }
                        }
            ...
            <div class="stat-card fade-in">
                ...
                <div class="stat-number">{{ $answers->count() - $approvedCount }}</div>
            ...
            <div class="stat-card fade-in">
                ...
                <div class="stat-number">
                    @php
                        $uniqueUsers = collect($answers)->pluck('user_id')->unique()->count();
                        echo $uniqueUsers;
                    @endphp
                </div>
    ...
    <div class="search-filter-section">
        <div class="position-relative">
            ...
            <input type="text" class="search-input" id="searchInput" placeholder="Cari jawaban berdasarkan pertanyaan, user, atau status...">
        </div>
    </div>

    {{-- Premium Answers Table --}}
    <div class="premium-card">
                ...
                <table class="premium-table">
                    <thead>
                        <tr>
                            ...
                            <th><i class="bi bi-chat-square-quote me-2"></i>Jawaban</th>
                            <th><i class="bi bi-person-fill me-2"></i>User</th>
                    ...
                    <tbody>
                        @foreach($answers as $answer)
                            @php
                                ...
                                } elseif (isset($answer->is_approved)) {
                                    $status = $answer->is_approved ? 'approved' : 'pending';
                                } elseif (isset($answer->approved)) {
                                    $status = $answer->approved ? 'approved' : 'pending';
                                }
                                ...
                                if (isset($answer->user) && $answer->user) {
                                    $userName = $answer->user->name ?? 'User Anonim';
                                    $userEmail = $answer->user->email ?? 'user@example.com';
                                    $userInitial = strtoupper(substr($userName, 0, 1));
                                }
                                
                                // Get question info
                                $questionText = 'Pertanyaan tidak tersedia';
                                if (isset($answer->question) && $answer->question) {
                                    $questionText = $answer->question->question ?? $answer->question->title ?? 'Pertanyaan tidak tersedia';
                                } elseif (isset($answer->question_text)) {
                                    $questionText = $answer->question_text;
                                }
                            @endphp
                        <tr class="answer-row">
                            <td>
                                <div class="question-title">Pertanyaan Terkait:</div>
                            ...
                            <td>
                                <div class="answer-preview">
                                    {{ $answer->answer ?? $answer->content ?? 'Jawaban tidak tersedia' }}
                                </div>
                            </td>
                            <td>
                                <div class="user-info">
                            ...
                            <td>
                                    ...
                                    @if($status === 'pending')
                                        <form action="{{ route('admin.answers.approve', $answer->id) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn-action approve" title="Setujui Jawaban">
                                                <i class="bi bi-check-circle-fill"></i>
                                            </button>
                                        </form>
                                    @endif
                                    ...
                                    {{-- Delete Answer --}}
                                    <form action="{{ route('admin.answers.destroy', $answer->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jawaban ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-action delete" title="Hapus Jawaban">
        ...
        @else
            <div class="empty-state">
                ...
                <p>Belum ada jawaban yang diberikan dalam sistem. Tunggu sampai user mulai menjawab pertanyaan kesehatan yang diajukan.</p>
                <a href="{{ route('admin.questions.index') }}" class="premium-btn">
                    <i class="bi bi-chat-square-text-fill"></i>Lihat Pertanyaan
                </a>
            </div>
        @endif
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(() => {
                ...
                tableRows.forEach((row, index) => {
                    ...
                    const answerText = row.querySelector('.answer-preview').innerText.toLowerCase();
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            const answerPreview = this.closest('tr').querySelector('.answer-preview').innerText;
            const truncatedAnswer = answerPreview.length > 80 ? 
                answerPreview.substring(0, 80) + '...' : answerPreview;
            
            const confirmed = confirm(`Yakin ingin menghapus jawaban ini?\n\n"${truncatedAnswer}"\n\nTindakan ini tidak dapat dibatalkan.`);
    ...
    // Confirmation dialog for approve buttons
    const approveButtons = document.querySelectorAll('.btn-action.approve');
    approveButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            const confirmed = confirm('Yakin ingin menyetujui jawaban ini?');
    ...
    if (window.performance && window.performance.mark) {
        window.performance.mark('answers-table-loaded');
    }
    ...
    // Uncomment if you want auto-refresh every 60 seconds
    /*
    setInterval(() => {
        if (document.visibilityState === 'visible') {
            // Check for new answers or status updates
